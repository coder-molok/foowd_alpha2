...
(function(root, factory) {
  ...
  var $, $ias, $img, $init, $margin, $preWindos, $wSize, Gobj, LoadPop, PrevWindow, elgg, preview, scale, setStyle, start;
  Gobj = {};
  elgg = require('elgg');
  $ = require('jquery');
  $init = {};
  ...
  Gobj.initialize = function(obj) {
    ...
    document.getElementById($init.fileId).addEventListener('change', function(e) {
  ...
  start = function() {
    var ar, decimals, div, i, j, len, oldCrop, src, tmp, val, variable, x;
    decimals = 100000;
    ...
    for (i = j = 0, len = ar.length; j < len; i = ++j) {
      ...
      if (val === '') {
        switch (tmp) {
          case 'x1':
            oldCrop.x1 = Math.round(($img.width - scale.l) / 2);
            break;
          case 'x2':
            oldCrop.x2 = Math.round(scale.l + oldCrop.x1);
            break;
          case 'y1':
            oldCrop.y1 = Math.round(($img.height - scale.l) / 2);
            break;
          case 'y2':
        ...
        if (tmp === 'x1' || tmp === 'x2') {
          oldCrop[tmp] = val * $img.width;
        }
        if (tmp === 'y1' || tmp === 'y2') {
          oldCrop[tmp] = val * $img.height;
        }
      }
    }
