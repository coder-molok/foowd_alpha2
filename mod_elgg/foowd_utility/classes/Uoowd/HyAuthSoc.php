...
class HyAuthSoc{
	...
	public function __construct(){
		...
		if(!empty($not_yet)){ 
			...
			foreach($not_yet as $y){
				$href = '?auth='.$y;
				if($y === 'Google') $href = \Uoowd\Param::pageDNS()->social . $href;
				$str .= "<li><a class=\"soc-" . strtolower($y) . "\"href=\"$href\"></a></li>";
			}
			echo '<ul class="soc">' . $str . '</ul>';
		}
	...
	public function makeAuthentication($provider){
		if($provider == "Facebook"){
			...
			$url = \Uoowd\Param::pageDNS()->indexauth;
		}
	...
	public function setConfig($redirect = null){
		if(is_null($redirect)){
			...
			if($_GET['auth'] === 'Google') $redirect = \Uoowd\Param::pageDNS()->indexauth;
